...
type FavoriteServiceHTTPServer interface {
	...
	// GetFavoriteList expose to http client
...
func RegisterFavoriteServiceHTTPServer(s *http.Server, srv FavoriteServiceHTTPServer) {
	r := s.Route("/")
	r.GET("/favorite/list", _FavoriteService_GetFavoriteList0_HTTP_Handler(srv))
	r.POST("/favorite/action", _FavoriteService_FavoriteAction0_HTTP_Handler(srv))
}
