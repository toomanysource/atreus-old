...
type RelationRepo interface {
	GetFollowList(context.Context, int64) ([]*User, error)
	GetFollowerList(context.Context, int64) ([]*User, error)
...
func (uc *RelationUsecase) GetFollowList(ctx context.Context, id int64, tokenString string) ([]*User, error) {
...
func (uc *RelationUsecase) GetFollowerList(ctx context.Context, id int64, tokenString string) ([]*User, error) {
	token, err := uc.parseToken(uc.config.Http.TokenKey, tokenString)
	if err != nil {
		return nil, err
	}
...
// Action 关注和取消关注
func (uc *RelationUsecase) Action(ctx context.Context, tokenString string, id int64, actionType int32) error {
	token, err := uc.parseToken(uc.config.Http.TokenKey, tokenString)
	if err != nil {
