...
func (m *MockCommentRepo) CreateComment(ctx context.Context, videoId uint32, commentText string, userId uint32) (*Comment, error) {
	comment := &Comment{
		Id: autoCount,
		User: &User{
			Id:   userId,
...
func (m *MockCommentRepo) DeleteComment(ctx context.Context, videoId, commentId uint32, userId uint32) (*Comment, error) {
...
func TestCommentUsecase_CommentAction(t *testing.T) {
	_, err := usecase.CommentAction(
		context.TODO(), 1, 0, 1, "test", token)
	assert.Nil(t, err)
	_, err = usecase.CommentAction(
		context.TODO(), 1, 1, 2, "", token)
	assert.Nil(t, err)
}

func TestCommentUsecase_GetCommentList(t *testing.T) {
	comments, err := usecase.GetCommentList(context.TODO(), token, 1)
	assert.Nil(t, err)
